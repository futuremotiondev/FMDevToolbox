Function ConvertTo-FlatObjectAzure {
    <#
    .SYNOPSIS
    ...
    $Object3 | ConvertTo-FlatObjectAzure
    .NOTES
    From https://evotec.xyz/powershell-converting-advanced-object-to-flat-object/
    Based on https://powersnippets.com/ConvertTo-FlatObjectAzure/
    #>
    [CmdletBinding()]
    Param (
    ...
    End {
        If ($PSBoundParameters.ContainsKey("OutputObject")) {
            ...
            $Iterate = [ordered] @{}
            if ($null -eq $Object) {
                #Write-Verbose -Message "ConvertTo-FlatObjectAzure - Object is null"
            }
            ...
            If ($Iterate.Keys.Count) {
                foreach ($Key in $Iterate.Keys) {
                    ConvertTo-FlatObjectAzure -Objects @(, $Iterate["$Key"]) -Separator $Separator -Base $Base -Depth $Depth -Path ($Path + $Key) -OutputObject $OutputObject
                }
            }
            else {
        ...
        elseif ($InputObjects.Count -gt 0) {
            foreach ($ItemObject in $InputObjects) {
                ...
                ConvertTo-FlatObjectAzure -Objects @(, $ItemObject) -Separator $Separator -Base $Base -Depth $Depth -Path $Path -OutputObject $OutputObject
